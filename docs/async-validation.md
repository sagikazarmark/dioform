...
That conservatism does not retain submit-triggered verdicts across intents. Each **Validator Source** has one current result and async run identifier, so its stored submit-triggered verdict belongs to the most recent attempting **Submit Intent**. After another intent runs the source, per-intent availability and per-intent visible errors no longer report the earlier intent's verdict; the earlier intent's next submit attempt starts fresh submit-scoped validation instead of reusing the other intent's result. See [ADR-0039](adr/0039-decline-per-intent-retention-of-submit-triggered-validator-results.md).
